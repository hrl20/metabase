...
(deftest save-table-xray-dashboard-test
  (testing "Should be able to save a Dashboard generated by X-Rays"
    (mt/dataset test-data
      ;; The `dashboard/save` endpoint opens its own transaction. Refer to `save-transient-dashboard!`.
      ;; That transaction commits independently of an outer transaction. Therefore a `:rollback-only`
      ;; wrapper does not remove the new rows. This test must delete the new rows itself.
