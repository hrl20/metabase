...
(deftest save-table-xray-dashboard-test
  (testing "Should be able to save a Dashboard generated by X-Rays"
    (mt/dataset test-data
      ;; `dashboard/save` opens its own nested transaction (see `save-transient-dashboard!`), which commits
      ;; independently of this outer transaction -- a `:rollback-only` wrapper here does NOT undo the writes, so we
      ;; have to clean them up explicitly instead.
